...
const _json = function (path, content) {
    ...
    _eval = function (cmd, db, last, tx) {
        var __csv = function (path, opts, cols, tfnc) {
            var args = Array.prototype.slice.apply(arguments);
            args.unshift(db._dyn);
            return _csv.apply(null, args);
        };
                    ...
                    completer: function (linePartial, cb) {
                        if (linePartial.indexOf('db.') == 0) {
                            var tables = _.collect(_.filter(_.keys(db),
                                    function (key) {
                                        return key.indexOf(linePartial.replace('db.', '')) == 0;
                                    }),
                                function (res) {
                                    return 'db.' + res;
                                });
                ...
                (function ask() {
                    ...
                    rl.question('> ', function (answer) {
                        ...
                        if (answer.indexOf('show collections') > -1) {
                            _.filter(_.keys(db), function (key) {
                                return !!db[key] && !!db[key].find;
                            }).forEach(function (c) {
                                console.log(c);
                            });
                            ask();
                        ...
                        try {
                            var time = process.hrtime(),
                                ...
                                doneres = _.wrap(_doneres, function (done) {
                                    if (printed && end) done();
                                }),
                                elapsed = function () {
                            ...
                            if (promise.error)
                                promise.error(_ask(function (err) {
                                    ...
                                    else if (err.code == 'updatedsinceread')
                            ...
                            if (promise.count)
                                promise.count(_ask(function (count) {
                                    console.log(('\r' + count).green);
                                    elapsed();
                                }));
                            ...
                            if (promise.transaction)
                            ...
                            else if (promise.committed)
                                promise.committed(function () {
                                    tx = undefined;
                                    console.log('transaction committed'.green);
                                    elapsed();
                                    ask();
                                });
                            else if (promise.rolledback)
                                promise.rolledback(function (competing) {
                                    tx = undefined;
                                    if (competing) console.log('transaction rolled back'.red); else console.log('transaction rolled back'.green);
                                    elapsed();
                                    ask();
                                });
                            else if (promise.clean)
                                promise.clean(function (obj) {
                                    console.log(util.inspect(obj, {depth: null}));
                                    ask();
                                });
                            else if (promise.result) {
                                last = undefined;
                                promise.result(function (obj) {
                                    last = obj;
                                    _print(obj, function () {
                                        elapsed();
                                        ask();
                                ...
                                promise.success(_ask(function () {
                                    console.log('done!'.green);
                                    elapsed();
                                }));
                            else
                                _ask(function () {
                                    console.log(util.inspect(promise, {depth: null}));
                                })();
                        }
                        catch (ex) {
