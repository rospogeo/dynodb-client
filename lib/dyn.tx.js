        ...
        _perform = function (dynI, ctx, p, obj, op, _apply) {
            var _id = _ctxId(ctx),
                    ...
                    dynI().getNT(function (item) {
                        ...
                        else // the item is locked
                        {
                            ...
                            if (item._tx == tx._id) // ok: lock already acquired
                                _locked(item);
                            else if (item._tx != tx._id) // mm: lock acquired by another transaction
                ...
                _save = function (p, item, cb) {
                    var t = dynC();
                    t.putNT(_.extend({}, item, {
                        _id: dyn.ctx.hash.value,
                        _item: dyn.ctx.range.value
                    }), cb, {exists: false})
                        .consumed(p.trigger.consumed)
                        .error(function (err) {
                            if (err.code != 'found')
                ...
                _add = function (p, cb) {
                    txR().getNT(function (_txR) {
                        if (_txR.state == 'pending') {
                            ...
                            t.putNT(_.extend({_txOp: op}, {update: JSON.stringify(obj)}, {
                                _id: dyn.ctx.hash.value,
                                _item: dyn.ctx.range.value
                            }), cb)
                                .consumed(p.trigger.consumed)
                    ...
                    async.forEach(_.keys(items),
                        ...
                        function (err) {
                            if (err)
                                ...
                                cb(_.extend(_.filter(items, function (i) {
                                    return !!i;
                                }), {next: items.next}));
                                done();
                            }
