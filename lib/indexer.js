    ...
    index.rebuild = function (window) {
        ...
        sindex.scan({attrs: ['_hash', '_range'], limit: limit})
            .on('data', function (items) {
                process.stdout.write(('\r' + (dcnt += items.length)).red);
            })
            .pipe(del)
            .on('finish', function () {
                console.log();
                ...
                dyn.stream(table._dynamo.TableName)
                    ...
                    .on('data', function (items) {
                        process.stdout.write(('\r' + (pcnt += items.length)).yellow);
                    })
    ...
    index.update = function (item, op) {
        ...
        if (index.indexable(item)) {
            ...
            if (index.indexable(item._old)) {
                ...
                if (elem._hash != old._hash || elem._range != old._range)
