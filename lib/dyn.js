...
const _catch = require('./utils/catch'),
    _arr = require('./utils/array'),
    _value = require('./utils/value'),
    _attr = require('./utils/attr'),
    _error = require('./utils/error'),
    _item = require('./utils/item'),
    _toItem = require('./utils/to-item'),
...
module.exports = function (opts, tx) {
    ...
    var _dyn = new AWS.DynamoDB(_.pick(opts, ['endpoint'])),
        dyn = {ctx: {}, queue: {error: []}},
        _promise = require('./utils/promise'),
        _results = require('./utils/results');
            ...
            _dyn.getItem(query,
                function (err, data) {
                    if (data && data.ConsumedCapacity)
                        promise.trigger.consumed({
                            table: query.TableName,
                            read: data.ConsumedCapacity.CapacityUnits,
                            write: 0
                        });
    ...
    dyn.query = function (cb, opts) {
        ...
        query.KeyConditions[dyn.ctx.hash.attr] = {
            AttributeValueList: _.collect(_arr(dyn.ctx.hash.value), _value),
            ComparisonOperator: dyn.ctx.hash.operator
        };

        if (dyn.ctx.range)
            query.KeyConditions[dyn.ctx.range.attr] = {
                AttributeValueList: _.collect(_arr(dyn.ctx.range.value), _value),
                ...
                function (err, data) {
                    ...
                    if (data && data.ConsumedCapacity)
                        promise.trigger.consumed({
                            table: query.TableName,
                            read: 0,
                            write: data.ConsumedCapacity.CapacityUnits
                        });
    ...
    dyn.updateItem = function (opts, cb) {
        ...
        _.keys(opts.update).forEach(function (attr) {
            ...
            query.AttributeUpdates[attr] = {
                Action: upd.action,
                Value: upd.value !== undefined ? _value(upd.value) : null
            };
        });
                        ...
                        return function () {
                            _dyn.batchWriteItem({RequestItems: UnprocessedItems},
                                function (err, data) {
                                    ...
                                    else if (_.keys(data.UnprocessedItems).length)
    ...
    dyn.create = function (cb, opts) {
        ...
        var query = {
            ...
            ProvisionedThroughput: {
                ReadCapacityUnits: opts.throughput.read || 1,
                WriteCapacityUnits: opts.throughput.write || 1
            },
            TableName: dyn.ctx.table
        };

        query.AttributeDefinitions.push({
            AttributeName: dyn.ctx.hash.attr,
            AttributeType: dyn.ctx.hash.value
        });

        query.KeySchema.push({
            AttributeName: dyn.ctx.hash.attr,
            KeyType: 'HASH'
        });
        ...
        if (dyn.ctx.range) {
            query.AttributeDefinitions.push({
                AttributeName: dyn.ctx.range.attr,
    ...
    dyn.scan = function (cb, opts) {
        ...
        var query = {TableName: dyn.ctx.table},
            _filterField = function (field) {
                return {
                    AttributeValueList: _.collect(field.values, _value),
    ...
    dyn.stream = function (table) {
        ...
        methods.writable = function (write) {
            var wstream = new Stream(),
                ...
                _ops = function (items) {
                    var ops = {};
                    ops[table] = _.collect(items, function (item) {
                        return {op: op, item: item};
                    });
                    return ops;
                };
